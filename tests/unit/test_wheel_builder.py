...
from pip._internal.cache import _CacheabilityJudge
...
def test_should_cache(req: ReqMock, expected: bool) -> None:
    assert _CacheabilityJudge.should_cache(cast(InstallRequirement, req)) is expected
...
def test_should_cache_git_sha(tmpdir: Path) -> None:
    ...
    assert _CacheabilityJudge.should_cache(cast(InstallRequirement, req))

    # a link not referencing a sha should not be cached
    url = "git+https://g.c/o/r@master#egg=mypkg"
    req = ReqMock(link=Link(url), source_dir=repo_path)
    assert not _CacheabilityJudge.should_cache(cast(InstallRequirement, req))
